...
int do_connect(int argc, char **argv)
{
  ...
  { /* Determine destination */
    ...
    if ((flx = find_dest(argv[1], NULL)) != NULL)
    { /* Check FlexNet */
      k = 1;
    ...
    else if ((ax = find_route(argv[1], NULL)) != NULL)
    { /* Check AX25 Links */
      k = 1;
      ...
      case CONN_TYPE_DIGI:
      {
        argv[k++] = ax->dev;
        argv[k++] = ax->dest_call;
        while ((k - 3) < AX25_MAX_DIGIS && ax->digis[k - 3][0] != '\0')
        {
          argv[k] = ax->digis[k - 3];
          k++;
        }
    ...
    else if ((ax = find_mheard(argv[1])) != NULL)
    { /* Check Mheard database */
      k = 1;
      /*	K2MF supplied code	*/
  ...
  else if ((User.ul_type == AF_AX25) && (!stay))
  {
    axio_flush(NodeIo);
    axio_end_all();
    logout_user();
